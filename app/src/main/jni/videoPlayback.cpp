    ...
    if ((*env).ExceptionCheck ()) {
	  ...
	  GST_DEBUG ("Initialization not complete");
	    if (!data->native_window)
               GST_DEBUG ("native windows failed");
         if (!data->main_loop)
               GST_DEBUG ("main loop failed");
  }
}
...
static void gst_native_configPipeline (JNIEnv *env, jobject thiz, jstring str) {
	jboolean isCopy;
...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  if (!custom_data_field_id) {
	  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "The calling class does not implement native_custom_data");
	  return JNI_FALSE;
  }
  if (!set_message_method_id) {
	  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "The calling class does not implement setMessage");
	  return JNI_FALSE;
  }
  if (!notify_state_method_id) {
	  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "The calling class does not implement notifyState");
	  return JNI_FALSE;
  }
  if (!on_gstreamer_initialized_method_id) {
	  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "The calling class does not implement onGStreamerInitialized");
	  return JNI_FALSE;
  }
  if (!set_error_method_id) {
	  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "The calling class does not implement setError");
	  return JNI_FALSE;
  }

  __android_log_print (ANDROID_LOG_ERROR, "GStreamer", "gst_native_class_init OK");
...
static JNINativeMethod native_methods[] = {
  ...
  { "nativeConfigPipeline", "(Ljava/lang/String;)V", (void *) gst_native_configPipeline},
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if ((*vm).GetEnv( (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
    __android_log_print (ANDROID_LOG_ERROR, "UDPStreamer", "Could not retrieve JNIEnv");
    return 0;
  }
  jclass klass = (*env).FindClass ( "com/peetie/rpidrone/gstreamer/VideoPlaybackFull");
  (*env).RegisterNatives ( klass, native_methods, G_N_ELEMENTS(native_methods));

  pthread_key_create (&current_jni_env, detach_current_thread);
  char *version_utf8 = gst_version_string();
  __android_log_print (ANDROID_LOG_VERBOSE, "UDPStreamer", "GSTREAMER VERSION: %s",version_utf8);
  g_free (version_utf8);;
